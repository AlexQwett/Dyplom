  ...
  return (
    ...
    >
      ...
      <div className="sm:ml-16 sm:w-48">
        {/* TITLE */}
        <h3 className="font-medium text-accent-content">{title}</h3>
        {/* COMPANY */}
        <h4 className="mt-2 text-sm text-accent-content">
          Торгова марка: { brandName }
        </h4>
        <h4 className="mt-2 text-sm text-accent-content">
          Фасування: { selectedSize }
        </h4>
      </div>
      <div className="sm:ml-12">
        ...
        <div className="form-control max-w-xs">
          <label htmlFor="amount" className="label p-0">
            <span className="label-text text-accent-content">Кількість</span>
          </label>
          <input
            name="number"
            id="amount"
      ...
      {/* PRICE */}
      <p className="font-medium sm:ml-auto text-accent-content">{ (price * amount).toFixed(2) } грн.</p>
    </article>
  );
};
