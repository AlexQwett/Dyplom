...
export const shopLoader = async ({ request }) => {
  ...
  const filterObj = {
    brand: params.brand ?? "Всі",
    category: params.category ?? "Всі",
    date: mydate ?? "",
    pet: params.pet ?? "Всі",
    order: params.order ?? "",
  ...
  let parameter = (`?_start=${(filterObj.current_page - 1) * 10}&_limit=10`) + // pre defined that limit of response is 10 & page number count 1
    (filterObj.brand !== 'Всі' ? `&brandName=${filterObj.brand}` : "") +
    (filterObj.category !== 'Всі' ? `&category=${filterObj.category}` : "") +
    (filterObj.pet !== 'Всі' ? `&pet=${filterObj.pet}` : ``) +
  ...
  try {
    ...
    if (filterObj.order && !(filterObj.order === "За зростанням" || filterObj.order === "За зростанням ціни")) data.sort((a, b) => b.price.current.value - a.price.current.value)
  ...
  return (
    <>
      <div className="max-w-7xl mx-auto mt-5">
        <Filters />
        {productLoaderData.productsData.length === 0 && <h2 className="text-accent-content text-center text-4xl my-10">Нічого не знайдено</h2>}
